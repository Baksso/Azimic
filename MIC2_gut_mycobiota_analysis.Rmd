...
#plate7 <- read.csv(file = "plate7_repeat_results_combined.csv")
...
fungclust_lma$estimate <- exp(fungclust_lma$estimate)
fungclust_lma$std.error <- exp(fungclust_lma$std.error)
...
ordered_coef_fclust <- fungclust_lma2
  
ordered_coef_fclust$term <- recode_factor(ordered_coef_fclust$term, 
...
fclust_regress <- ggplot(data = ordered_coef_fclust, aes(estimate, term)) + geom_point()+ 
  geom_vline(xintercept = 1, lty=3)+
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high, width = 0.25))+
  theme_classic() +
  geom_text(label=ordered_coef_fclust$P.value, 
            nudge_y = 0.2, 
            nudge_x = c(2))+
  labs(y=NULL, x= "Odds of having fungCluster_1 or fungCluster_2", tag = "a")+
  scale_x_continuous(limits = c(-6.0, 10), n.breaks = 9)+
  theme(axis.title = element_text(size = 12, vjust = 1),
        axis.text.x = element_text(size = 10),
